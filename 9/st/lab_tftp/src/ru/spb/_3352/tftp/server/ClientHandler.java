...
/**
 * However this class is called ClientHandler it is meant to be the class that
 * handles the actual client tftp transfers.
 * 
 * @author marco
 */
public class ClientHandler
{
    ...
    /**
     * The private field for storage of the timeout setting for the
    ...
    /**
     * Connects the clienthandler to the client, packages send do not need to
     * contain a address. If they do they are overwritten by the ones of the
     * connected socket.
     * 
     * @return
     */
    ...
    /**
     * disconnects from the client, any packet send after a disconnect needs to
     * contain a destination address and port, because nne is available on the
     * socket.
     */
    ...
    /**
     * This method returns the IP address and port number of the client this
     * class is talking to.
     */
    ...
    /**
     * This method is an internal helper function that checks if the received
     * ack packet matches the expectations. That means that the opcode and
     * blocknumbers are expected.
     * 
    ...
    // {
    // tftpP = tftpSock.read();
    // // set timout back because we gave enough time to clean up stuffed
    // channel
    // if (stuffedLink)
    // {
    ...
    // }
    //
    // // all other is spam and when too many of this crap is give up, and do
    // not signal
    // if (spamcount++ > 5)
    // {
    // return null;
        ...
        boolean useOptions )
    {
        ...
        // Because the sendLastAckForDataPacket call can take ages, we close the
        // output stream before sending this ack
        try
        {
            os.close();
        }
    ...
    /**
     * This method will send a file to the client after a read request Because
     * the client buffers are slow to process the package in case of the LSDM it
     * will set a special flag when a package is resent, as soon as the data is
     * acknowledge the next data packet will be sent, it will check and see the
     * special flag, clear the flag and sent for once the new data packet with a
     * doubled time out this gives the LSDM time to also process the possibly
     * delayed data package
     */
        ...
        boolean useOptions )
    {
        ...
        // clear any duplicate packages in socket queue before processing the
        // new request
        tftpSock.clear();
        ...
        {
            System.out.println( "CODING ERRO? this case should be fetched in DataTransfer!" );
            return false;
        }
        catch ( InstantiationException e )
        {
        ...
        do
        {
            ...
            {
                read = bis.read( data );
                // when eof is encountered and last block equaled BLOCK_SIZE we
                // need to
                // send zero block
                if ( read < 0 )
                {
