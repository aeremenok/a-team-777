...
public class Scanner {
	...
	void CheckLiteral() {
		String lit = t.val;
		if (lit.compareTo("interface") == 0) t.kind = 4;
		else if (lit.compareTo("final") == 0) t.kind = 5;
		else if (lit.compareTo("static") == 0) t.kind = 6;
		else if (lit.compareTo("class") == 0) t.kind = 11;
	...
	Token NextToken() {
		...
		while (!done) {
			...
			switch (state) {
				...
				case 2:
					if (ch >= '0' && ch <= '9') {tval[tlen++] = (char)ch; NextCh(); state = 3; break;}
					else {t.kind = noSym; done = true; break;}
				case 3:
					if (ch >= '0' && ch <= '9') {tval[tlen++] = (char)ch; NextCh(); state = 3; break;}
					else if (ch == 'e') {tval[tlen++] = (char)ch; NextCh(); state = 4; break;}
					else {t.kind = 2; done = true; break;}
				case 4:
					if (ch == '-') {tval[tlen++] = (char)ch; NextCh(); state = 5; break;}
					else {t.kind = noSym; done = true; break;}
				case 5:
				...
				case 7:
					if (ch <= 9 || ch >= 11 && ch <= 12 || ch >= 14 && ch <= '!' || ch >= '#' && ch <= 65535) {tval[tlen++] = (char)ch; NextCh(); state = 7; break;}
					else if (ch == '"') {tval[tlen++] = (char)ch; NextCh(); state = 8; break;}
					else {t.kind = noSym; done = true; break;}
				case 8:
				...
				case 13:
					...
					else if (ch == '.') {tval[tlen++] = (char)ch; NextCh(); state = 2; break;}
					else if (ch == 'e') {tval[tlen++] = (char)ch; NextCh(); state = 4; break;}
					else {t.kind = 2; done = true; break;}
				case 14:
