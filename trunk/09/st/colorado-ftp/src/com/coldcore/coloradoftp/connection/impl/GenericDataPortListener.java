  ...
  public void run() {
    while (bound) {
      ...
      try {
        ...
        log.debug("New incoming data connection (IP "+ip+")");
        ...
        if (controlConnection == null) {
          log.warn("No control connection found for an incoming data connection");
        ...
        log.warn("Failed to accept a connection (ignoring)", e);
        try {
          dataConnection.destroyNoReply();
        } catch (Throwable ex) {}
        try {
